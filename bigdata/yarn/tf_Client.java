...
public class Client {
    ...
    // Source and destination directory pairs to output
    private String[] outputPairs = new String[]{};
    // Relative mounted directory pairs of docker extracted from shell_env
    private List<String> relativeMountPairs = new ArrayList<>();
    // Writable absolute mounted directory of docker extracted from shell_env
    private Set<String> writableAbsoluteMountSet = new HashSet<String>();
    ...
    // No. of parameter servers, used only when 
    // runtime = DSConstants.RUNTIME_DISTRIBUTED_TENSORFLOW
    private int numParameterServers = 1;
    // No. of trainers, used only when 
    // runtime = DSConstants.RUNTIME_DISTRIBUTED_TENSORFLOW
    private int numTrainers = 1;
    ...
    private long clientTimeout = 0; // non-positive means no limit
    ...
    public static final String TAR_PATH = "TAR_PACKAGE";
    ...
    public Client(Configuration conf) throws Exception {
        this("distributedshell.ApplicationMaster", conf);
    }
    ...
    Client(String appMasterMainClass, Configuration conf) {
        ...
        opts.addOption("timeout", true, "Application timeout in seconds");
        ...
        opts.addOption("master_resource_profile", true,
                "Resource profile for the application master");
        opts.addOption("runtime", true, "Runtime of training.");
        opts.addOption("shell_command", true, "Shell command to be executed by "
                + "the Application Master. Must be specified.");
        opts.addOption("tar", true,
                "Location of the tar package which contains scripts to be "
                        + "executed. Optional.");
        opts.addOption("shell_args", true, "Extra args for the script."
        ...
        opts.addOption("output", true,
                "File or directory to output from containers. Specified as src:dest pair."
                        + " You can repeat this option but each option describes one pair.");
        ...
        opts.addOption("container_resource_profile", true,
                "Resource profile for the shell command");
        opts.addOption("num_containers", true,
                "No. of containers on which the shell command needs to be executed");
        opts.addOption("num_parameter_servers", true,
                "No. of parameter servers");
        ...
        opts.addOption("flow_name", true,
                "Flow name which the distributed shell " + "app belongs to");
        opts.addOption("flow_version", true,
                "Flow version which the distributed " + "shell app belongs to");
        opts.addOption("flow_run_id", true,
                "Flow run ID which the distributed " + "shell app belongs to");
    ...
    public boolean init(String[] args) throws ParseException, IOException {
        ...
        if (cliParser
                .hasOption("keep_containers_across_application_attempts")) {
        ...
        amMemory = Integer
                .parseInt(cliParser.getOptionValue("master_memory", "-1"));
        amVCores = Integer
                .parseInt(cliParser.getOptionValue("master_vcores", "-1"));
        if (cliParser.hasOption("master_resources")) {
            Map<String, Long> masterResources = parseResourcesString(
            ...
            for (Map.Entry<String, Long> entry : masterResources.entrySet()) {
                if (entry.getKey().equals(ResourceInformation.MEMORY_URI)) {
                    amMemory = entry.getValue();
                        ...
                        .equals(ResourceInformation.VCORES_URI)) {
        ...
        switch (runtime) {  // runtime对应参数检查
            ...
            case DSConstants.RUNTIME_SHARED_TENSORFLOW:
                if (cliParser.hasOption("num_containers")
                        || cliParser.hasOption("num_parameter_servers")
                        || cliParser.hasOption("num_trainers")) {
                    throw new IllegalArgumentException(
                            "Option num_containers or num_parameter_servers or num_trainers "
                                    + "is not used in distributedshell runtime。");
                }
                break;
            case DSConstants.RUNTIME_DISTRIBUTED_TENSORFLOW:
                if (cliParser.hasOption("num_containers")) {
                    throw new IllegalArgumentException(
                            "Option num_containers is not used in "
                                    + "distributedtensorflow runtime。 Please use "
                                    + "num_parameter_servers and num_trainers instead.");
                }
                break;
            case DSConstants.RUNTIME_HOROVOD:
                if (cliParser.hasOption("num_parameter_servers")
                        || cliParser.hasOption("num_trainers")) {
                    throw new IllegalArgumentException(
                            "Option num_parameter_servers or num_trainers "
                                    + "is not used in distributedtensorflow runtime。 "
                                    + "Please use num_containers instead.");
                }
                break;
            default:
                throw new IllegalArgumentException(
                        "Runtime " + runtime + " is unsupported");
        }
        ...
        if (cliParser.hasOption("shell_command")) {
            ...
            throw new IllegalArgumentException(
                    "No shell command to be executed by application master");
        }
        if (cliParser.hasOption("tar")) {
            tar = cliParser.getOptionValue("tar");
        }
        ...
        if (cliParser.hasOption("shell_env")) {
            ...
            for (String env : envs) {
                ...
                // for mounts
                if (key.equals("YARN_CONTAINER_RUNTIME_DOCKER_MOUNTS")) {
                    val = buildMounts(val);
                }
                shellEnv.put(key, val);
            }
        }
        shellCmdPriority = Integer
                .parseInt(cliParser.getOptionValue("shell_cmd_priority", "0"));

        if (cliParser.hasOption("output")) {
            outputPairs = cliParser.getOptionValues("output");
            for (String pair : outputPairs) {
                String[] segs = pair.split(":");
                if (segs.length != 2) {
                    throw new IllegalArgumentException("Invalid output pairs");
                }
                if (!writableAbsoluteMountSet.contains(segs[1])) {
                    throw new IllegalArgumentException(segs[1]
                            + " is not a writable absolute mounted directory.");
                }
            }
        }

        if (cliParser.hasOption("container_type")) {
            String containerTypeStr = cliParser
                    .getOptionValue("container_type");
            if (Arrays.stream(ExecutionType.values())
                    .noneMatch(executionType -> executionType.toString()
                            .equals(containerTypeStr))) {
                throw new IllegalArgumentException(
                        "Invalid container_type: " + containerTypeStr);
            }
        ...
        containerMemory = Integer
                .parseInt(cliParser.getOptionValue("container_memory", "-1"));
        containerVirtualCores = Integer
                .parseInt(cliParser.getOptionValue("container_vcores", "-1"));
        if (cliParser.hasOption("container_resources")) {
            Map<String, Long> resources = parseResourcesString(
        ...
        containerResourceProfile = cliParser
                .getOptionValue("container_resource_profile", "");
        numContainers = Integer
                .parseInt(cliParser.getOptionValue("num_containers", "1"));
        numParameterServers = Integer.parseInt(
                cliParser.getOptionValue("num_parameter_servers", "1"));
        numTrainers = Integer
                .parseInt(cliParser.getOptionValue("num_trainers", "1"));

        if (numContainers < 1) {
            throw new IllegalArgumentException(
                    "Invalid no. of containers specified,"
                            + " exiting. Specified numContainer="
                            + numContainers);
        }
        if (numParameterServers < 1) {
            throw new IllegalArgumentException(
                    "Invalid no. of parameter servers specified,"
                            + " exiting. Specified numParameterServers="
                            + numParameterServers);
        }
        if (numTrainers < 1) {
            throw new IllegalArgumentException(
                    "Invalid no. of trainers specified,"
                            + " exiting. Specified numTrainers=" + numTrainers);
        }
        ...
        clientTimeout = 1000
                * Integer.parseInt(cliParser.getOptionValue("timeout", "0"));

        attemptFailuresValidityInterval = Long.parseLong(cliParser
        ...
        if (cliParser.hasOption("flow_run_id")) {
            try {
                flowRunId = Long
                        .parseLong(cliParser.getOptionValue("flow_run_id"));
        ...
        if (cliParser.hasOption("docker_client_config")) {
            dockerClientConfig = cliParser
                    .getOptionValue("docker_client_config");
        }

        if (cliParser.hasOption("application_tags")) {
            String applicationTagsStr = cliParser
                    .getOptionValue("application_tags");
    ...
    public boolean run() throws IOException, YarnException {
        ...
        // manager
        long maxMem = appResponse.getMaximumResourceCapability()
                .getMemorySize();
        ...
        if (amVCores > maxVCores) {
            LOG.info(
                    "AM virtual cores specified above max threshold of cluster. "
        ...
        // The tar package has to be made available on the final container(s)
        ...
        if (!tar.isEmpty()) {
            Path tarSrc = new Path(tar);
            String tarPathSuffix = appName + "/" + appId.toString() + "/"
                    + TAR_PATH;
            Path tarDst = new Path(fs.getHomeDirectory(), tarPathSuffix);
        ...
        LOG.debug("Tar package is stored in HDFS as: " + hdfsTarLocation);
        ...
        if (!relativeMountPairs.isEmpty()) {
            String[] pairs = new String[relativeMountPairs.size()];
            addToLocalResources(fs, null, relativeMountPairsPath,
                    appId.toString(), localResources,
                    StringUtils.join(relativeMountPairs.toArray(pairs), " "));
        }

        if (outputPairs.length > 0) {
            addToLocalResources(fs, null, outputPairsPath, appId.toString(),
                    localResources, StringUtils.join(outputPairs, " "));
        }
        ...
        // scripts
        env.put(DSConstants.DISTRIBUTED_SHELL_TAR_LOCATION, hdfsTarLocation);
        env.put(DSConstants.DISTRIBUTED_SHELL_TAR_TIMESTAMP,
                Long.toString(hdfsTarTimestamp));
        env.put(DSConstants.DISTRIBUTED_SHELL_TAR_LEN, Long.toString(hdfsTarLen));
        if (domainId != null && domainId.length() > 0) {
            env.put(DSConstants.DISTRIBUTED_SHELL_TIMELINE_DOMAIN, domainId);
        }
        ...
        if (containerVirtualCores > 0) {
            vargs.add("--container_vcores "
                    + String.valueOf(containerVirtualCores));
        }
        if (!containerResources.isEmpty()) {
            Joiner.MapJoiner joiner = Joiner.on(',').withKeyValueSeparator("=");
            vargs.add(
                    "--container_resources " + joiner.join(containerResources));
        }
        if (containerResourceProfile != null
                && !containerResourceProfile.isEmpty()) {
            vargs.add(
                    "--container_resource_profile " + containerResourceProfile);
        }
        vargs.add("--runtime " + runtime);
        if (DSConstants.RUNTIME_DISTRIBUTED_SHELL.equals(runtime)) {
            vargs.add("--num_containers 1");
        } else if (DSConstants.RUNTIME_DISTRIBUTED_TENSORFLOW.equals(runtime)) {
            vargs.add("--num_parameter_servers "
                    + String.valueOf(numParameterServers));
            vargs.add("--num_trainers " + String.valueOf(numTrainers));
        } else if (DSConstants.RUNTIME_HOROVOD.equals(runtime)) {
            vargs.add("--num_containers " + String.valueOf(numContainers));
        }

        if (placementSpec != null && placementSpec.length() > 0) {
            // Encode the spec to avoid passing special chars via shell
            // arguments.
            String encodedSpec = Base64.getEncoder().encodeToString(
                    placementSpec.getBytes(StandardCharsets.UTF_8));
        ...
        LOG.debug("Completed setting up app master command "
                + command.toString());
        List<String> commands = new ArrayList<String>();
        commands.add(command.toString() + ";");

        // commands.add(String.format("chmod -R +020 %s/..;",
        ...
        if (dockerClientConfig != null) {
            dockerCredentials = DockerClientConfigHandler
                    .readCredentialsFromConfigFile(new Path(dockerClientConfig),
                            conf, appId.toString());
        }
        ...
        if (rmCredentials != null || dockerCredentials != null) {
            ...
            ByteBuffer tokens = ByteBuffer.wrap(dob.getData(), 0,
                    dob.getLength());
    ...
    /**
     * Replace relative mounts with absolute mounts and return the latter.
     * Record relative mounts and writable absolute mounts
     *
     * @param mountString
     * @return replacedMountString
     */
    private String buildMounts(String mountString) throws IOException {
        StringBuilder mountBuilder = new StringBuilder();

        String[] mounts = mountString.split(",");
        for (String mount : mounts) {
            String[] segs = mount.split(":");
            if (segs.length != 3) {
                throw new IllegalArgumentException("Invalid mount as " + mount);
            }
            if (mountBuilder.length() > 0) {
                mountBuilder.append(',');
            }
            // check writable
            if (segs[2].equals("rw")) {
                File path = new File(segs[0]);
                if (!Files.isWritable(path.toPath())) {
                    throw new IllegalArgumentException(String.format(
                            "Directory %s is not writable, can not mount as rw",
                            segs[0]));
                }
            }
            // treat relative and absolute mount differently
            if (segs[1].charAt(0) != '/') {
                relativeMountPairs.add(segs[0] + ":" + segs[1]);
                mountBuilder.append(segs[0]).append(':').append(segs[0])
                        .append(':').append(segs[2]);
                continue;
            } else if (segs[2].equals("rw")) {
                writableAbsoluteMountSet.add(segs[1]);
            }
            mountBuilder.append(mount);
        }

        return mountBuilder.toString();
    }
    ...
    private boolean monitorApplication(ApplicationId appId)
        ...
        while (true) {
            ...
            if (clientTimeout > 0 && System
                    .currentTimeMillis() > (clientStartTime + clientTimeout)) {
                LOG.info(
    ...
    private void addToLocalResources(FileSystem fs, String fileSrcPath,
        ...
        if (fileSrcPath == null) {
            ...
            try {
                ostream = FileSystem.create(fs, dst,
        ...
        LocalResource scRsrc = LocalResource.newInstance(
                URL.fromURI(dst.toUri()), LocalResourceType.FILE,
    ...
    private void prepareTimelineDomain() {
        ...
        try {
            // TODO: we need to check and combine the existing timeline domain
            // ACLs,
            // but let's do it once we have client java library to query
            // domains.
    ...
    private void setAMResourceCapability(
        ...
        if (amMemory < -1 || amMemory == 0) {
            throw new IllegalArgumentException("Invalid memory specified for"
                    + " application master, exiting. Specified memory="
                    + amMemory);
        }
        if (amVCores < -1 || amVCores == 0) {
            throw new IllegalArgumentException(
                    "Invalid virtual cores specified for"
        ...
        if (appContext.getAMContainerResourceRequests() == null) {
            ...
            amResourceRequests.add(ResourceRequest.newInstance(
                    Priority.newInstance(amPriority), "*",
        ...
        appContext.getAMContainerResourceRequests().get(0)
    ...
    private void validateResourceTypes(Iterable<String> resourceNames,
        ...
        for (String resourceName : resourceNames) {
            if (!resourceTypes.stream()
                    .anyMatch(e -> e.getName().equals(resourceName))) {
        ...
        for (String resource : resourcesStr.trim().split(",")) {
            ...
            if (!resource.matches("^[^=]+=\\d+\\s?\\w*$")) {
                throw new IllegalArgumentException("\"" + resource
                        + "\" is not a " + "valid resource type/amount pair. "
            ...
            String valueWithoutUnit = value
                    .substring(0, value.length() - units.length()).trim();
